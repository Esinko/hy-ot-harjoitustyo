...
class TileWidget(QtCore.QObject, QtWidgets.QGraphicsRectItem):
    ...
    def paint(self, painter, option, widget):
        ...
        if self.hasFocus():
            ...
            painter.drawEllipse(center, self.edit_circle_radius, self.edit_circle_radius)
        else:
            self.setZValue(1)
...
class EditorGraphicsView(QtWidgets.QGraphicsView):
    ...
    def __init__(self, is_preview: bool = False):
        ...
        self.setSceneRect(-10000, -10000, 20000, 20000)  # Large space for "infinite" area
        if not is_preview:
        ...
        self.coord_label.setStyleSheet("color: white; background: rgba(0,0,0,0.5); padding: 2px; font-size: 12px;")
        self.coord_label.setAlignment(QtCore.Qt.AlignmentFlag.AlignLeft | QtCore.Qt.AlignmentFlag.AlignVCenter)
    ...
    def resizeEvent(self, event):
        self.coord_label.move(5, self.viewport().height() - self.coord_label.height() - 5)
    ...
    def drawForeground(self, painter: QtGui.QPainter, rect: QtCore.QRectF):
        ...
        if self.focusedTileWidget and not self.focusedTileWidget.hasFocus():
            QtCore.QTimer.singleShot(0, lambda: self.focusedTileWidget.setFocus(QtCore.Qt.OtherFocusReason))
    ...
    def dragEnterEvent(self, event: QtGui.QDragEnterEvent): self._canDrag(event)
    ...
    def dropEvent(self, event: QtGui.QDropEvent):
        if event.mimeData().hasText():
            if event.mimeData().text() == "BDM; new_tile":
                ...
                self.addElementEvent.emit(AddElementEvent(int(scene_pos.x() // self.element_size), int(scene_pos.y() // self.element_size), 1, 1))
                event.accept()
                ...
                pos = event.position().toPoint()
                scene_pos = self.mapToScene(pos)
                self.moveElementEvent.emit(MoveElementEvent(target_tile, int(scene_pos.x() // self.element_size), int(scene_pos.y() // self.element_size)))
                event.accept()
        else:
            event.ignore()
    ...
    def render(self, elements: List[Element]):
        ...
        for element in elements:
            # Create tile
            tile = TileWidget(element.x * self.element_size, element.y * self.element_size, self.element_size, self.element_size, id=element.id)
            tile.focusEvent.connect(lambda _, t=tile: self._setFocusedTile(t)) # The parameter magic here forces python to use the tile from this iteration of the loop!
            ...
            # Create tile name
            label = GraphicsLabel(text=element.name, backgroundColor="#000", color="white")
            ...
            label = GraphicsLabel(text=f"x: {element.x}, y: {element.y}", backgroundColor="#000", color="white")
            label.setParentItem(tile)
            label_rect = label.boundingRect()
            label.setPos(
                (element.x * self.element_size) + self.element_size - label_rect.width() - 10,
                (element.y * self.element_size) + self.element_size - label_rect.height() - 10
            )